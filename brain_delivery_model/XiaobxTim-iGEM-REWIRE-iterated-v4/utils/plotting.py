...
from models.editing.module5 import (
...
def plot_module5_states(results: Dict[str, Any], output_dir: str | Path) -> Path:
    ...
    plt.xlabel("Time (h)")
    plt.ylabel("Amount / Level")
    plt.title("Module 5: PUF-APOBEC Editing States")
    plt.legend()
    plt.tight_layout()
...
def plot_module5_fluxes(results: Dict[str, Any], output_dir: str | Path) -> Path:
    ...
    plt.xlabel("Time (h)")
    plt.ylabel("Flux")
    plt.title("Module 5: PUF-APOBEC Editing Fluxes")
    plt.legend()
    plt.tight_layout()
...
def plot_module5_metrics(results: Dict[str, Any], output_dir: str | Path) -> Path:
    ...
    if "on_target_editing_fraction" in metrics:
        plt.plot(t, metrics["on_target_editing_fraction"], label="on_target_editing_fraction")
    if "off_target_editing_fraction" in metrics:
        plt.plot(t, metrics["off_target_editing_fraction"], label="off_target_editing_fraction")
    for name in [
        "apoe4_fraction",
        "apoe3_like_fraction",
        "apoe2_like_fraction",
        "mixed_edit_fraction",
        "local_bystander_burden",
        "puf_mismatch_burden",
        "deaminase_background_burden",
        "ldlr_binding_risk_proxy",
    ]:
        if name in metrics:
            plt.plot(t, metrics[name], label=name)
